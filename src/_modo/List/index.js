...
const List = (props) => {
	...
	if (items instanceof Array) {
		for (let i = 0; i < items.length; i++) {
			elements.push(itemRender(items[i], i, value, onChange, idPrefix));
		}
	} else if (items instanceof Map) {
		for (let [key, val] of items) {
			elements.push(itemRender(val, key, value, onChange, idPrefix));
		}
	} else if (items instanceof Object) {
		const keys = Object.keys(items);
		for (let i = 0; i < keys.length; i++) {
			const key = keys[i];
			elements.push(itemRender(items[key], key, value, onChange, idPrefix));
		}
	}
