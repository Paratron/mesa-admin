...
const Calendar =
	({
		...
		onlyDate(workDate);
		const valueDate = new Date(workDate.getTime());
		...
		for(let week = 0; week < 6; week++){
			...
			for(let day = 0; day < 7; day++){
				...
				if(workDate.getTime() === valueDate.getTime()){
					dayClasses.push('mdo-selected');
				}
			...
			rows.push(<div className="mdo-calendar-row" key={week + ''}>{row}</div>);
		}

		return (
			<div className="mdo-calendar">
				<div className="mdo-header">
					{weekdays.map(day => <div key={day} className={`mdo-day mdo-${day}`}>{day}</div>)}
				</div>
